...
namespace CarBus
{
    public partial class ucChauffeur : UserControl
    {
        ...
        private void btnOpslaan_Click(object sender, EventArgs e)
        {
            ...
            if (btnOpslaan.Name == "btnAanmaken")
            {
                ...
                ChauffeurManagement.addChauffeur(txtNaam.Text, txtVoornaam1.Text, txtVoornaam2.Text, txtInDienst.Text,
                    ...
                    txtFax.Text, txtEmail.Text, txtIdentiteitskaart.Text, txtGarage.Text, txtBadge.Text, txtMemo.Text, 
        ...
        {
            string errStr = "";
            if (txtGeboorteDatum.Text.Replace(" ", "") == "//")
            {
                errStr = "U moet een geboortedatum invullen.";
                e.Cancel = true;
            }
            else
            {
            ...
            errorProvider1.SetError(txtGeboorteDatum, errStr);
        }
        ...
        {            
            String errStr = "";
            if (txtAard_rijbewijs.Text.Trim().Length == 0)
            {
                errStr = "U moet een identiteitskaart type invullen.";
                e.Cancel = true;
            }
            else
            {
            ...
            errorProvider1.SetError(txtAard_rijbewijs, errStr);
        }
        ...
        {
            String errStr = "";
            if (txtGeboorteplaats.Text.Trim().Length == 0)
            {
                errStr = "U moet een geboorteplaats invullen.";
                e.Cancel = true;
            }
            else
            {
            ...
            errorProvider1.SetError(txtGeboorteplaats, errStr);
        }

        private void txtBankrekening_Validating(object sender, CancelEventArgs e)
        {
            string errStr = "";   // Initializes and assumes valid data as default
            if (Validation.IsBankrekeningnummer(txtBankrekening.Text) == false)
            {
            ...
            // Hiding or showing of the ErrorProvider depends on the 2nd string arguement
            // Empty string --> hides; Non-empty ---> shows
            errorProvider1.SetError(txtBankrekening, errStr);
        }

        private void txtRijbewijs_Validating(object sender, CancelEventArgs e)
        {
            string errStr = "";
            if (txtRijbewijs.Text.Replace(" ", "") == "-")
            {
                errStr = "U moet een rijbewijs nummer invullen.";
                e.Cancel = true;
            }
            else
            {
            ...
            errorProvider1.SetError(txtRijbewijs, errStr);
        }
